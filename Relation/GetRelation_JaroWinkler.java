...
public class GetRelation_JaroWinkler {
	...
	public static ArrayList<String> similar_company(ArrayList<Firma> data) {
		...
		ergebnis.add("Methode: 		similar_company_jarowinkler");
					...
					for (y = 1; y < data.size() - 1; y++) {
						...
						if (naechster.getFirma() != null) {
							...
							if (company2 != null
								...
								if (Compare.compare_jarowinkler(company1, company2) > 0.97) {
	...
	public static ArrayList<String> similar_company_same_city(
		...
		ergebnis.add("Datum:	   " + df.format(now.getTime()));
		ergebnis.add("Wortfilter: 	Gemeinde");
		ergebnis.add("Methode: 	similar_company_same_city_jarowinkler");
		ergebnis.add("ergebnisfiller");
		ergebnis.add("");
				...
				if (company1 != null && !company1.contains("Gemeinde")
					...
					for (y = 1; y < data.size() - 1; y++) {
						...
						if (naechster.getFirma() != null) {
							...
							String stadt2 = Clean_Firma.clean_firma(naechster.getFirma().trim());
							if (company2 != null
									&& !company2.contains("Gemeinde")
									&& stadt2 != null) {
								// Alle Ergebnisse mit einer Levensthein Distanz
								...
								if (Compare.compare_jarowinkler(company1, company2) > 0.93) {
	...
	public static ArrayList<String> similar_company_same_plz(
		...
		ergebnis.add("Methode: 		similar_company_same_plz_jarowinkler");
		ergebnis.add("ergebnisfiller");
		ergebnis.add("");
					...
					for (y = 1; y < data.size() - 1; y++) {
						...
						if (naechster.getFirma() != null) {
							...
							if (company2 != null && plz2 != null) {
								if (Compare.compare_jarowinkler(company1, company2) > 0.9) {
									if (plz1.equals(plz2)) {
										ergebnis.add(company1 + ", " + company2);
										++x;
									}
	...
	public static ArrayList<String> similar_contact_same_city(
		...
		ergebnis.add("Methode: 		similar_contact_same_city_jarowinkler");
		ergebnis.add("ergebnisfiller");
		ergebnis.add("");
		...
		while (0 < data.size()) {
			...
			if (firma1 != null && !firma1.contains("Gemeinde")
				...
				for (y = 1; y < data.size() - 1; y++) {
					...
					String firma2 = Clean_Firma.clean_firma(naechster.getFirma().trim());
					if (firma2 != null && !firma2.contains("Gemeinde")
						...
						// geschrieben
						if (Compare.compare_jarowinkler(name1, name2) > 0.96) {
							if (stadt1.equalsIgnoreCase(stadt2)) {
